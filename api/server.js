...
app.get('/api/stats', async (req, res) => {
  ...
  try {
    connection = await mysql.createConnection({
      ...
      port: 3307,
      user: 'benchmark2026',
      password: 'Benchmark941!!',
      database: 'benchmark-mysql'
    });
    ...
    // 5. Clinical Outcomes & Improvements
    const [testRecords] = await connection.query(`
      SELECT pts.patient_id, pts.test_date, ptr.test_id, tl.name AS test_name, tc.name AS category_name, ptr.left, ptr.right, ptr.no_laterality
    ...
    for (const row of testRecords) {
      ...
      if (!patientTests[key]) {
        patientTests[key] = { test_name: row.test_name, category: row.category_name, records: [] };
      }
    ...
    for (const key in patientTests) {
      ...
      if (data.records.length > 1) {
        ...
        if (!testImprovements[testName]) {
          testImprovements[testName] = { category: data.category, leftChanges: [], rightChanges: [], noLatChanges: [] };
        }
        if (first.left !== null && last.left !== null) testImprovements[testName].leftChanges.push(last.left - first.left);
        if (first.right !== null && last.right !== null) testImprovements[testName].rightChanges.push(last.right - first.right);
        if (first.no_laterality !== null && last.no_laterality !== null) testImprovements[testName].noLatChanges.push(last.no_laterality - first.no_laterality);
      }
    }
    ...
    for (const testName in testImprovements) {
      ...
      if (n >= 5) { // Filter to tests with at least 5 patients to show reliable data
        improvementsData.push({
          testName,
          category: data.category,
          patients: n,
          leftAvg: avg(data.leftChanges),
          rightAvg: avg(data.rightChanges),
    ...
    res.json({
      metrics: {
        ...
        conversionRate,
        arpu,
        avgSessionsPerClinician,
        avgPatientsPerClinician,
        longitudinalPct
      },
      charts: {
        userGrowth: userGrowth.reverse(),
        testDomains: testTypes
      },
      outcomes: {
        tests: improvementsData,
        proms: promsData
      }
    });
